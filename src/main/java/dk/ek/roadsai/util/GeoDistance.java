...
public final class GeoDistance {
    ...
    // helper to order stations along a route (from start→end == left→right)
    public static double progressAlongPolylineM(double lat, double lon, List<List<Double>> lineLonLat) {
        ...
        for (int i = 0; i < lineLonLat.size() - 1; i++) {
            ...
            if (dToSeg < bestDistance) {
                bestDistance = dToSeg;
                bestProgressOnSegment = segLenM * proj.tClamped;
                // snapshot progress up to segment start
                progressBefore = lengthUntilIndexM(lineLonLat, i);
            }
        }
    ...
    // helper class to hold projection results
    private static final class Projection {
        final double distanceM; // shortest distance from point to segment
        final double tClamped;  // 0..1 position along segment from a to b
    ...
    private static Projection projectPointOnSegment(double lat, double lon, List<Double> a, List<Double> b) {
        ...
        // Use average latitude for x scaling (reduces distortion for Iceland latitudes)
        ...
        // Convert projected point back to geographic and compute great-circle distance
